...
- browsecomp_compute_score: Uses GPT-4.1 for browsing comprehension evaluation
"""
...
from mirorl.monitor import get_or_init_wandb
...
def get_judge_llm_name():
    return os.environ.get("JUDGE_LLM_NAME", "gpt-4.1")
...
def extract_boxed_answer(full_result):
    ...
    if isinstance(
        ...
        boxed_result = (
            "ERROR_UNEXPECTED_RESULT_TYPE"  # Handle non-string results if they occur
        )
    return boxed_result
...
def normalize_number_string(number_str: str) -> float:
    ...
    try:
        ...
        print(
            f"llm_judge.py: normalize_number_string: String {number_str} cannot be normalized to number str."
        )
        return float("inf")
...
def simpleqa_compute_score(
    ...
    try:
        ...
        if wandb_run is not None:
            wandb_run.alert(
                title="SimpleQA Evaluation Failed",
...
confidence: The extracted confidence score between 0|%| and 100|%| from [response]. Put 100 if there is no confidence score available."""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a precise and objective answer verification assistant.",
                },
        ...
        if wandb_run is not None:
            wandb_run.alert(
                title="GPT-4.1 Evaluation Failed",
    ...
    try:
        ...
        if any(sep in ground_truth for sep in [",", ";"]):
            ...
            if len(gt_elements) != len(answer_elements):
                warnings.warn(
                    "gaia_naive_compute_score: Answer lists have different lengths, returning False.",
                    UserWarning,
                )
                return 0.0
...
class ResponsesSampler(SamplerBase):
    ...
    def __init__(
        ...
        self.client = OpenAI(api_key=get_openai_api_key(), base_url=get_openai_base_url(), timeout=30, max_retries=3)
        self.model = model
...
def browsecomp_compute_score(
    data_source: str,
    solution_str: str,
    ground_truth: str,
    extra_info: Optional[Dict] = None,
) -> float:
    """Compute BrowseComp score using LLM grader."""
    try:
        answer = extract_boxed_answer(solution_str)
        question = extra_info.get("question", "") if extra_info else ""

        grader = ResponsesSampler(model=get_judge_llm_name())
        prompt = BROWSECOMP_GRADER_TEMPLATE.format(
            question=question, response=answer, correct_answer=ground_truth
        )
        ...
        # print("#### response.response_text", response.response_text)
        # print("#### match", match)
        # print("#### anw", anw)
        ...
        print(
            f"llm_judge.py: browsecomp_compute_score: BrowseComp evaluation failed: {e}"
        )
        if wandb_run is not None:
            wandb_run.alert(
                title="BrowseComp Evaluation Failed",
...
def cascade_compute_score(
    ...
    gaia_score = gaia_naive_compute_score(
        data_source, solution_str, ground_truth, extra_info
    )

    # If GAIA naive found a match, return that score
    if gaia_score > 0.0:
        # print(f"Cascade: GAIA naive succeeded with score {gaia_score}")
        return gaia_score

    # If GAIA naive failed, fallback to BrowseComp (LLM-based evaluation)
    # print("Cascade: GAIA naive failed (0.0), falling back to BrowseComp")
    browsecomp_score = browsecomp_compute_score(
        data_source, solution_str, ground_truth, extra_info
    )
    # print(f"Cascade: BrowseComp returned score {browsecomp_score}")
