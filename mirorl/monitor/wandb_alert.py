...
def get_or_init_wandb():
    ...
    if (
        env_vars.get("ENABLE_WANDB_ALERT", "False") == "False"
        or os.environ.get("WANDB_API_KEY") is None
    ):
        return None

    if wandb.run is None:
        wandb.init(
            project=env_vars.get("WANDB_PROJECT"), name=env_vars.get("WANDB_NAME")
        )
    return wandb.run
    ...
    def update_latest_iteration(self, iteration: int) -> int:
        ...
        if self.latest_iteration > 1:
            ...
            if cur_error_counts > pre_error_counts * self.alert_threshold:
                print(
                    f"MCPToolChecker Step {self.latest_iteration} Alert: {cur_error_counts=} >= "
                    f"{pre_error_counts=} * {self.alert_threshold}",
                    flush=True,
                )
                if self.wandb_run is not None:
                    self.wandb_run.alert(
                        ...
                        text=(
                            f"MCP Tool Call failed total times: {cur_error_counts}, "
                            f"Detail Error Type: {self.error_counts[self.latest_iteration]}"
                        ),
...
class HangChecker:
    ...
    def __init__(self, config):
        self.latest_iteration_path = os.path.join(
            config.trainer.default_local_dir, "latest_iteration.txt"
        )
