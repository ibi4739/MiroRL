...
from mirorl.utils.reward_score import default_compute_score
from mirorl.utils.reward_score.llm_judge import (
    extract_boxed_answer,
    SIMPLEQA_EVALUATION_PROMPT,
)


async def batch_compute_scores_async(
    client,
    data_source_list,
    response_str_list,
    ground_truth_list,
    ...
    async def compute_single_score(i, retry_count=0):
        ...
        try:
            ...
            # breakpoint()
            prompt = SIMPLEQA_EVALUATION_PROMPT.format(
                extra_info.get("question", ""),
                ground_truth,
                extract_boxed_answer(response_str),
            )
            ...
            print(
                f"Error computing score for sample {i} (attempt {retry_count + 1}): {e}"
            )

            # Retry if we haven't reached max_retry
            if retry_count < max_retry:
                print(
                    f"Retrying sample {i} (attempt {retry_count + 2}/{max_retry + 1})"
                )
                ...
                raise Exception(
                    f"Failed to compute score for sample {i} after {max_retry + 1} attempts"
                ) from e
    ...
    for i in range(0, len(data_source_list), batch_size):
        ...
        for j, result in enumerate(batch_results):
            if isinstance(result, Exception):
                print(
                    f"Exception in batch {i // batch_size}, sample {batch_indices[j]}: {result}"
                )
    ...
    if len(results) != len(response_str_list):
        raise ValueError(
            f"Results length mismatch: got {len(results)}, expected {len(response_str_list)}"
        )
...
class ToolRewardManager:
    ...
    def __init__(
        ...
        self.accuracy_reward_weight = (
            accuracy_reward_weight  # Store the weight for the accuracy reward
        )
        self.tool_format_reward_weight = (
            tool_format_reward_weight  # Store the weight for the tool reward
        )
        self.gate_tool_format_reward = gate_tool_format_reward  # Store the gating flag
        self.async_process = (
            async_process  # whether use async process to compute reward
        )
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        if self.async_process:
            ...
            print(
                "Start computing accuracy reward with async api, this may take a while..."
            )
            print(
                f"Async configuration: batch_size={self.batch_size}, max_retry={self.max_retry}"
            )
            try:
                ...
                end_time = time.time()
                print(
                    f"Async batch processing completed in {end_time - start_time:.2f} seconds"
                )
        ...
        for i in range(len(data)):
            ...
            if self.gate_tool_format_reward:
                # if gate_tool_format_reward is True, the tool format reward will not be given
                # if the final answer is not correct, even the tool format is correct.
        ...
        # save the question, response, ground_truth, accuracy_reward, tool_format_reward, combined_reward to a jsonl file
        rollout_mask = [
            acc != 1 * self.accuracy_reward_weight
            for acc in reward_extra_info["accuracy_reward"]
        ]
        data.batch["rollout_mask"] = torch.tensor(rollout_mask, dtype=torch.bool)
        if return_dict:
            return {
    ...
    def check_mcp_format(self, prompt_str, response_str, verbose=False):
                ...
                for tool_name, raw_schema in tool_defs:
                    ...
                    try:
                        ...
                        if match:
                            ...
                            required_fields = "FAILED TO PARSE: fallback failed"

                        # Fallback: extract property types manually
                        prop_types = {}
                        prop_block = re.search(
                            r"'properties'\s*:\s*{(.*?)}\s*,?\s*'required'",
                            raw_schema,
                            re.DOTALL,
                        )
                        if prop_block:
        ...
        def extract_tool_blocks(msg: str) -> List[str]:
            return re.findall(
                r"<use_mcp_tool>.*?</use_mcp_tool>", msg.strip(), re.DOTALL
            )
        ...
        def validate_mcp_tool_use(xml_str: str, tool_schemas: dict) -> dict:
            ...
            try:
                server_name = (
                    re.search(r"<server_name>(.*?)</server_name>", xml_str)
                    .group(1)
                    .strip()
                )
                tool_name = (
                    re.search(r"<tool_name>(.*?)</tool_name>", xml_str).group(1).strip()
                )
                arguments_raw = re.search(
                    r"<arguments>\s*(\{.*\})\s*</arguments>", xml_str, re.DOTALL
                ).group(1)
            ...
            if tool_name not in tool_schemas:
                return {
                    "valid": False,
                    "errors": [f"Tool '{tool_name}' not found in schema."],
                }
            ...
            if schema["server_name"] != server_name:
                errors.append(
                    f"Server name mismatch: expected '{schema['server_name']}', got '{server_name}'"
                )
                ...
                for k, v in arguments.items():
                    ...
                    if expected_type:
                        if not isinstance(v, type_mapping.get(expected_type, object)):
                            errors.append(
                                f"Field '{k}' should be of type '{expected_type}', got '{type(v).__name__}'"
                            )
                    else:
        ...
        if not tool_specs:
            raise ValueError(
                "No tool schemas found. This may not be a valid system prompt."
            )

        format_score = 0.0
        # Check validity of mcp format
        blocks = extract_tool_blocks(response_str)
        if format_score != 1.0 and blocks:
            format_score = 1.0
